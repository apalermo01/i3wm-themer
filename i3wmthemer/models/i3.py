...
class I3Theme(AbstractTheme):
    ...
    def init_colors(self):
        ...
        print("i3theme keys = ", self.i3theme.keys())
        if "colors" not in self.i3theme:
            self.i3theme['colors'] = self.i3theme
        ...
        if 'background' in self.i3theme['colors']:
            ...
            print("USING COLORS.BACKGROUND")
        elif 'client.background' in self.i3theme['colors']:
            self.background = self.parse_color_line(self.i3theme['colors']['client.background'], self.x_resources)
            print("USING CLIENT.BACKGROUND")
        else:
            self.background = self.x_resources[XresourcesAttr.BACKGROUND.value]
        ...
        elif 'client.focused' in self.i3theme['colors']:
            self.focused = self.parse_color_line(self.i3theme['colors']['client.focused'], self.x_resources)
        else:
            self.focused = " ".join([self.x_resources[i] for i in focused_list])
        ...
        if 'unfocused' in self.i3theme['colors']:
            self.unfocused = self.parse_color_line(
                                ...
                                self.x_resources)
        elif 'client.unfocused' in self.i3theme['colors']:
            self.unfocused = self.parse_color_line(self.i3theme['colors']['client.unfocused'], self.x_resources)
        else:
            self.unfocused = " ".join([self.x_resources[i] for i in unfocused_list])
        ...
        if 'focused_inactive' in self.i3theme['colors']:
            self.inactive = self.parse_color_line(
                                ...
                                self.x_resources)
        elif 'client.focused_inactive' in self.i3theme['colors']:
            self.inactive = self.parse_color_line(self.i3theme['colors']['client.focused_inactive'], self.x_resources)
        else:
            self.inactive = " ".join([self.x_resources[i] for i in focused_inactive_list])

        if 'urgent' in self.i3theme['colors']:
            self.urgent = self.parse_color_line(self.i3theme['colors']['urgent'], self.x_resources)
        elif 'client.urgent' in self.i3theme['colors']:
            self.urgent = self.parse_color_line(self.i3theme['colors']['client.urgent'], self.x_resources)
        else:
            self.urgent = " ".join([self.x_resources[i] for i in urgent_list])
        ...
        elif 'client.placeholder' in self.i3theme['colors']:
            self.placeholder = self.parse_color_line(self.i3theme['colors']['client.placeholder'], self.x_resources)
        else:
            self.placeholder = " ".join([self.x_resources[i] for i in placeholder_list])
